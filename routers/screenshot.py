...
#
# ============================================================================
# ✅ NEW (Sep 2026 — capture-engine failures returned HTTP 500 with internals)
# ============================================================================
#   When the shared Chromium died or the sync Playwright API was touched from
#   the wrong thread, the raw text reached the user through the generic
#   handler at the bottom of create_screenshot():
#
#       500 "Screenshot failed: Browser.new_context: Target page, context or
#            browser has been closed"
#       500 "Screenshot failed: Cannot switch to a different thread"
#
#   Both are server-side, both are retryable, and neither is a 500 in the
#   sense the caller can act on. They now return 503 with a Retry-After header
#   and a message that says the site was not at fault. (greenlet.error is not a
#   Playwright exception type, so detection is by message, not by class.)
#
#   The root causes are fixed in services/screenshot_service.py — single
#   worker thread, browser relaunch, and a full-page capture budget.
#
# ✅ NEW (Sep 2026 — trimmed captures are reported to the caller)
#   A page taller than the capture budget is now trimmed rather than killing
#   Chromium. ScreenshotResponse carries truncated / document_height, plus
#   image_width / image_height, which the service has returned since Aug 2026
#   and this router was discarding.
#
# ✅ FIX (Sep 2026 — local storage fallback wrote to a cwd-relative path)
#   The R2 fallback wrote to Path("screenshots")/<user_id>, which resolves
#   against the process working directory. Run uvicorn from the repo root
#   instead of backend/ and the bytes land somewhere /screenshots does not
#   serve, so the URL returned 404. It now writes under the same
#   SCREENSHOTS_DIR the service and main.py's static mount use.
#
# ✅ FIX (Sep 2026 — retention_days read a key that does not exist)
#   tier_limits.get("screenshot_retention_days", 7) always returned 7, because
#   get_tier_limits() returns only screenshots / batch_requests / api_calls /
#   features. Same class of bug as BUG 4 below. It is now an explicit constant
#   that matches the R2 lifecycle rule and IMAGE_RETENTION_DAYS in the
#   frontend, instead of an accidental default.
#
# ============================================================================
...
#   check_user_screenshot_limit then evaluated `current < limit`, which raises
#   TypeError in Python 3, outside the try block → unhandled 500. Fix:
#   "unlimited" is checked explicitly before any arithmetic or comparison.
#
# ── BUG 2: Usage enforcement disagreed with the dashboard ───────────────────
#   Enforcement read user.usage_screenshots (a lifetime counter reset only on
#   Stripe invoice.paid, which Free users never generate); the dashboard
#   counted rows in the current period. Enforcement now calls
#   screenshots_used_this_period() — one source of truth.
#
# ── BUG 3: DELETE here bypassed the usage tombstone ─────────────────────────
#   There are TWO delete endpoints:
#       DELETE /api/v1/screenshots/{id}   (main.py)
#       DELETE /api/v1/screenshot/{id}    (this file)
#   Both now record the tombstone before deleting.
#
# ── BUG 4: max_width tier gate rejected every width above 1920 ──────────────
#   tier_limits.get("max_width", 1920) always returned the default, because
#   get_tier_limits() has no max_width key — so the Ultrawide preset failed
#   for every tier, and upgrading did not help. TIER_MAX_WIDTH below is a real,
#   visible policy.
#
# ============================================================================
# Previous fixes (all retained)
# ============================================================================
# ✅ FIX (Jul 2026 — PDF Tier Gate messaging): PDF is Pro+, message corrected.
# ✅ Phase 1: has_feature() gates, webhook retry + HMAC signing, js_warning.
# ✅ Phase 2: element_selector added to ScreenshotResponse (it was silently
#    dropped from every response even though the service returned it).
...
# Single source of truth for usage — enforcement and the dashboard read the
# same number, and deleting a screenshot no longer refunds quota.
...
# ✅ CHANGED (Sep 2026): SCREENSHOTS_DIR imported so the local fallback writes
# where main.py actually serves from.
from services.screenshot_service import screenshot_service, SCREENSHOTS_DIR
...
# ============================================================================
# Max viewport width per tier (BUG 4). 3840 is the ceiling ScreenshotRequest
# validates against, so paid tiers get the full documented range.
# ⚠️ batch.py carries a matching TIER_MAX_WIDTH. Change both together.
...
# ✅ NEW (Sep 2026): how long a stored image survives. This must match the R2
# lifecycle rule on the pixelperfect-screenshots bucket and
# IMAGE_RETENTION_DAYS in History.js / BatchJobs.js. It used to be read from a
# tier_limits key that does not exist, so it was always the fallback anyway —
# now it is honest about being a single global policy.
...
def is_unlimited(limit: Any) -> bool:
    ...
    models.py _env_limit() returns the STRING "unlimited" for Premium. Every
    comparison and subtraction involving a limit must go through this check
    first — see BUG 1.
    """
...
# ✅ NEW (Sep 2026): capture-engine failure detection.
#
# These are infrastructure faults, not caller errors. greenlet.error is raised
# by Playwright's sync API when it is touched from a foreign thread and is not
# a PlaywrightError subclass, so detection is by message rather than by type.
...
class ScreenshotResponse(BaseModel):
    ...
    ✅ Phase 2 (May 2026): element_selector — without this field Pydantic
       silently dropped it from every response.
    ✅ Sep 2026: image_width / image_height (true output pixels), and
       truncated / document_height for captures trimmed to the capture budget.
       width/height remain the VIEWPORT, as of the Aug 2026 device fix.
    """
    url: str
    ...
    element_selector: Optional[str] = None  # Phase 2: selector used for crop
    image_width: Optional[int] = None       # ✅ Sep 2026
    image_height: Optional[int] = None      # ✅ Sep 2026
    truncated: bool = False                 # ✅ Sep 2026
    document_height: Optional[int] = None   # ✅ Sep 2026
    message: Optional[str] = None           # ✅ Sep 2026: set when truncated
...
def check_user_screenshot_limit(
    ...
    ✅ REWRITTEN (Aug 2026 — BUGS 1 and 2). Usage comes from
    screenshots_used_this_period(), the same function /subscription_status
    uses, so enforcement and display cannot diverge. "unlimited" is handled
    before any comparison.

    NOTE: this signature takes `db`. Any other caller must be updated.
    """
    tier_limits = get_tier_limits(user.subscription_tier or "free")
    limit = tier_limits["screenshots"]

    # Pass the User, not the id — the period boundary depends on the user's
...
def increment_user_usage(user: User, db, usage_type: str = "screenshots"):
    ...
    ⚠️ NO LONGER AUTHORITATIVE (Aug 2026). Kept updated for backward
    compatibility; no limit decision is made from these columns. They do not
    reset for Free-tier users. Do not reintroduce enforcement based on them.
    """
...
async def send_webhook_notification(
    ...
    """
    POST screenshot completion payload to the caller's webhook URL.
...
async def create_screenshot(
    ...
    **JavaScript errors** are non-fatal. If `custom_js` throws, the screenshot
    still captures and `js_warning` contains the error.

    **Element selection** crops to the element's bounding box. Returns HTTP 400
    if the selector matches nothing or the element has zero size.

    **Very tall pages** are trimmed to the server's capture budget rather than
    failing; `truncated` and `document_height` report when that happened.
    ...
    **Webhook delivery** uses exponential-backoff retry (3 attempts) and
    ...
    if not can_use:
        # %-d is not portable (fails on Windows); %d is fine.
    ...
    # BUG 4: width gate reads a real per-tier policy. A device preset overrides
    # width entirely, so the gate is skipped in that case.
    ...
    try:
        ...
        # Service returns keys: filename, filepath, url, width, height,
        # viewport_*, image_*, requested_*, device_scale_factor, format,
        # full_page, dark_mode, file_size, created_at, js_warning,
        # element_selector, truncated, document_height.
        ...
        element_selector: Optional[str] = result.get("element_selector")
        image_width: Optional[int]      = result.get("image_width")
        image_height: Optional[int]     = result.get("image_height")
        truncated: bool                 = bool(result.get("truncated"))
        document_height: Optional[int]  = result.get("document_height") or None
        ...
        try:
            ...
            # ✅ FIX (Sep 2026): write under the directory main.py serves at
            # /screenshots instead of a cwd-relative "screenshots/" path.
            logger.warning("R2 upload failed, falling back to local storage: %s", upload_err)
            local_dir = SCREENSHOTS_DIR / str(current_user.id)
        ...
        # ── Database ──────────────────────────────────────────────────────────
        expires_at = datetime.utcnow() + timedelta(days=IMAGE_RETENTION_DAYS)

        # width/height are NOT NULL in models.py, and the column default does
        # not apply to an explicitly-set None, so persist what the service
        # actually produced with the request as a fallback.
        ...
        increment_user_usage(current_user, db, "screenshots")   # commits
        db.refresh(screenshot_record)
        db.refresh(current_user)

        logger.info(
            "✅ Screenshot created: %s for user %s (element=%s truncated=%s)",
        ...
        if request.webhook_url:
            background_tasks.add_task(
                ...
                screenshot_data={
                    ...
                    "truncated":       truncated,          # ✅ Sep 2026
                    "document_height": document_height,    # ✅ Sep 2026
                },
                secret=request.webhook_secret,
            )

        # BUG 1: usage block uses the unlimited-safe helpers.
        used_now = current + 1

        message: Optional[str] = None
        if truncated:
            message = (
                f"The page is {document_height}px tall, which exceeds the capture "
                f"limit at this width. The screenshot was trimmed. Reduce the "
                f"width or capture a specific section to get the whole page."
            )
        ...
        # ValueError, so check here too before calling it a client error.
        if _is_engine_failure(exc):
            logger.error("💥 Capture engine failure for %s: %s", request.url, exc)
            raise _engine_failure_http()
        logger.error("Capture validation error: %s", exc)
        raise HTTPException(status_code=400, detail=str(exc))
    except Exception as exc:
        # ✅ NEW (Sep 2026): greenlet.error and a dead browser arrive here.
        # They are retryable infrastructure faults, not 500s with internals.
        if _is_engine_failure(exc):
            logger.error(
                "💥 Capture engine failure for %s: %s", request.url, exc, exc_info=True
            )
            db.rollback()
            raise _engine_failure_http()
...
async def get_usage_stats(
    ...
    /subscription_status and as the enforcement path.
    """
    tier_limits = get_tier_limits(current_user.subscription_tier or "free")
    ...
    return {
        ...
        "limits": tier_limits,
        "max_width": _max_width_for(current_user),
        # ✅ NEW (Sep 2026): the retention window is a real policy constant now.
        "image_retention_days": IMAGE_RETENTION_DAYS,
        "period_start": current_period_start(current_user).isoformat(),
        # reset_date follows the user's Stripe billing anniversary on paid
        # tiers rather than the calendar. It used to echo user.usage_reset_at,
        # which is null for every Free user.
...
async def delete_screenshot(
    ...
    Both paths record the usage tombstone (BUG 3). Deleting a screenshot
    removes the artifact. It does NOT refund usage — the capture cost was
    already spent when Chromium rendered the page.
    """
    screenshot = (
        db.query(Screenshot)
        .filter(Screenshot.id == screenshot_id, Screenshot.user_id == current_user.id)
    ...
    # ✅ Must run BEFORE db.delete() — it reads created_at off the row, and
    # shares the caller's transaction so both commit together or not at all.
