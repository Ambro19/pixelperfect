...
#     Uses a one-off sync_playwright() context so it never touches the
...
class ScreenshotService:
    ...
    def _get_device_descriptor(self, device_key: str) -> Optional[Dict[str, Any]]:
        ...
        Uses a one-off sync_playwright() context — never touches the
        persistent browser instance. Returns None for unknown keys.
        """
        playwright_name = SUPPORTED_DEVICES.get(device_key)
        if not playwright_name:
            return None
        with sync_playwright() as pw:
            return pw.devices.get(playwright_name)
