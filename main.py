...
#
# 'self' on the FRONTEND is https://pixelperfectapi.net.
# The API lives at https://api.pixelperfectapi.net — a DIFFERENT origin.
# Browsers enforce CSP on the *page* origin, so every fetch() call to the
# API subdomain was blocked by the browser's CSP engine, producing
# "Failed to fetch" with no network request ever leaving the browser.
#
# Fix: explicitly allow the API subdomain in connect-src.
# Also added https://api.pixelperfectapi.net to img-src so screenshot
# previews served from the API aren't blocked either.
# =====================================================================
PROD_CSP = (
    "default-src 'self'; "
...
# ✅ FIX: DEV_ORIGINS now reads from CORS_DEV_ORIGINS env var so a LAN IP
# change (e.g. 192.168.1.185 → 192.168.1.158) never requires a code edit.
# Fallback list covers the two known IPs for continuity.
# To add a new IP without redeploying, set in your .env.local:
#   CORS_DEV_ORIGINS=http://192.168.1.158:3000,http://192.168.1.185:3000
...
if _dev_origins_env:
    ...
    DEV_ORIGINS = [
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "http://192.168.1.158:3000",   # current LAN IP (Mar 2026)
        "http://192.168.1.185:3000",   # previous LAN IP — kept for safety
    ]
...
# def _verify_required_routes(app: FastAPI) -> None:
#     """
#     Fail-fast (in dev) if critical routes are not registered as expected.
#     Directly targets the 405: POST /api/v1/batch/submit_file bug — logs
#     exactly what IS registered so you stop guessing at route mismatches.
#     """
#     wanted_path = "/api/v1/batch/submit_file"
#     wanted_method = "POST"
#     found_any = False
...
#     if not found_any:
#         logger.error("❌ Route not registered at all: %s", wanted_path)
#         # Print nearby candidates to quickly locate the real registered path
#         for r in app.router.routes:
#             rpath = getattr(r, "path", "") or ""
#             if "batch" in rpath and "submit" in rpath:
#                 logger.error("   nearby route: %s %s", getattr(r, "methods", None), rpath)
...
#       1. DB init + migrations  (restored — was accidentally commented out)
#       2. Route verification    (catches 405 root-cause at boot, not at runtime)
#       3. Screenshot service init
#     """
#     # --- 1. DB init + migrations ---
#     try:
#         initialize_database()
...
#     # --- 2. Route verification (raises in dev if POST /batch/submit_file missing) ---
#     _verify_required_routes(app)
...
# # Ensure trailing-slash requests redirect consistently.
# # Starlette default is True, but set explicitly so the behaviour is documented.
# app.router.redirect_slashes = True
...
# @app.delete("/api/v1/screenshots/{screenshot_id}")
# async def delete_screenshot(
#     screenshot_id: str,  # str, NOT int — PostgreSQL uses UUID primary keys
#     current_user: User = Depends(get_current_user),
#     db: Session = Depends(get_db),
# ):
#     """
...
#     Ownership enforced: users can only delete their own screenshots.
#     """
#     record = None

#     # Try integer lookup first (SQLite / integer PK)
#     try:
#         int_id = int(screenshot_id)
#         record = db.query(Screenshot).filter(
#             Screenshot.id == int_id,
...
#     # Fall back to string/UUID lookup (PostgreSQL)
#     if record is None:
#         record = db.query(Screenshot).filter(
#             Screenshot.id == screenshot_id,
#             Screenshot.user_id == current_user.id,
...
# # =====================================================================
# # FIXED: Subscription Status — direct DB count for all tiers
# # =====================================================================
# @app.get("/subscription_status")
# def subscription_status(
#     request: Request,
...
#     # Count directly from DB — works correctly for ALL tiers
#     now = datetime.now(timezone.utc)
#     period_start = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
...
#     # Batch count — non-fatal; falls back to legacy counter if model absent
#     batch_used = 0
#     try:
#         from models import BatchJob
#         batch_used = (
