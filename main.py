...
#         path = getattr(r, "path", None)
#         name = getattr(r, "name", "")
#         if path and "batch" in path:
#             print(f"[ROUTE] {methods} {path}  name={name}")
#     try:
#         await screenshot_service.initialize()
#         _set_screenshot_ready(True)
#     except Exception as e:
...
#     # ── ✅ FIX: Count directly from DB — works correctly for ALL tiers ────────
#     #
#     # ROOT CAUSE of zeros for Pro users:
#     #   Old code: `getattr(current_user, "usage_screenshots", 0)`
#     #   This reads counter columns (usage_screenshots, etc.) that were never
#     #   properly incremented after a user upgraded to Pro/Business tier.
#     #   Free users had these populated; Pro users did not.
#     #
#     # NEW: SELECT COUNT(*) from screenshots WHERE user_id=X AND this month.
#     #   ✓ Always accurate — reads actual rows, not a stale counter
#     #   ✓ Zero drift — no webhook or mid-flight crash can cause under-counting
#     #   ✓ Works immediately for existing users with no data migration needed
#     #
#     now = datetime.now(timezone.utc)
#     period_start = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
