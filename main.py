...
def _send_email_sync(name: str, email: str, subject: str, message: str, category: str):
    """Sends contact form email via Gmail SMTP. Runs as a background task."""
    ...
    if not smtp_user or not smtp_password:
        logger.warning("SMTP credentials not configured — contact email not sent.")
        return

    category_label = CATEGORY_LABELS.get(category, category.title())

    text_body = f"""
New contact form submission from PixelPerfect Screenshot API
...
<html><body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
  ...
  <div style="background:#f8fafc;border:1px solid #e2e8f0;border-top:none;padding:24px;border-radius:0 0 8px 8px;">
    ...
    <p style="color:#374151;white-space:pre-wrap;background:white;padding:16px;border-radius:6px;border:1px solid #e2e8f0;">{message}</p>
...
@app.post("/contact")
async def contact_form(payload: ContactFormRequest, background_tasks: BackgroundTasks):
    """
    Receives contact form submissions and emails them to onetechly@gmail.com.
    Email is sent in the background so the API response is instant.
    """
...
# def _send_email_sync(name: str, email: str, subject: str, message: str, category: str):
#     """Sends contact form email via Gmail SMTP. Runs in a thread pool."""
#     smtp_host     = os.getenv("SMTP_HOST", "smtp.gmail.com")
#     smtp_port     = int(os.getenv("SMTP_PORT", "587"))
#     smtp_user     = os.getenv("SMTP_USERNAME", "")
#     smtp_password = os.getenv("SMTP_PASSWORD", "")
...
#     logger.info(f"Contact email sent: [{category_label}] from {email}")


# @app.post("/contact")
# async def contact_form(payload: ContactFormRequest, background_tasks: BackgroundTasks): # pyright: ignore[reportUndefinedVariable]
#     """
#     Receives contact form submissions and emails them to onetechly@gmail.com.
#     Email is sent in the background so the response is instant.
#     """
#     # Basic validation
#     if not payload.name.strip() or not payload.email.strip() or not payload.message.strip():
#         raise HTTPException(status_code=422, detail="Name, email, and message are required.")
