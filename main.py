...
# # ✅ FIXED: Check if running on Render (production) instead of ENVIRONMENT variable
# IS_RENDER = os.getenv("RENDER") is not None

# if IS_RENDER:
#     # Production on Render - only public origins
#     allow_origins = PUBLIC_ORIGINS + ([FRONTEND_URL] if FRONTEND_URL else [])
#     logger.info("🌐 Running on Render - Using production CORS")
# else:
#     # Local development - include everything
#     allow_origins = PUBLIC_ORIGINS + DEV_ORIGINS + ([FRONTEND_URL] if FRONTEND_URL else [])
#     logger.info("🌐 Running locally - Using development CORS")
...
# # ✅ FIXED: Simplified OPTIONS handler (middleware handles headers)
# @app.options("/{path:path}")
# async def options_handler(path: str):
